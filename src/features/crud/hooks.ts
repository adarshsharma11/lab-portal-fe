...
export function useEntity<T extends Entity>(kind: Kind, id: string) { const service = serviceFor<T>(kind); return useQuery<ApiResponse<T | null>, Error, T | null>({ queryKey: [...keys[kind], id], queryFn: () => service.getById(id), select: (result) => result.data, enabled: Boolean(id && id !== "new") }); }
